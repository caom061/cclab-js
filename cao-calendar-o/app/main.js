...
function init(){
  ...
  //start
  changeMonth(_currentYear, _currentJSMonth, date.getDate());
}
...
function getHolidays(year, month, callback){
  $.ajax({
    ...
    dataType: "jsonp",
      data: {
        'ibio-url' : encodeURIComponent('http://holidayapi.com/v1/holidays?country=us&year=' + year + '&month=' + month)
      },
    success: callback
  });
}
...
function changeMonth(year, jsMonth, activeDay){
  getHolidays(year, jsMonth + 1, function(response){
    var holidays = buildHolidays(response.holidays || []);
    _app.collection.update(year, assembleMonth(year, jsMonth, holidays));
    _app.setActiveDay(activeDay);
  });
  setTitleMonth(year, jsMonth + 1);
}
...
function assembleMonth(year, month, holidays){
  var list = [];
  ...
  for(var i = 1; i <= daysInMonth; i++){
    ...
    list.push({month:date.getMonth(), date:date.getDate(), day:date.getDay(), adjacent:null, holidays:holidays[date.getDate()]});
  }
  //next week
  // 7 * 6 row
